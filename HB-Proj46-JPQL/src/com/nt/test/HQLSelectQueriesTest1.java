...
public class HQLSelectQueriesTest1 {
	...
	public static void main(String[] args) {
		...
		try{
			/*query=em.createQuery("select eno,ename,desg from Employee");
			...
			/*query=em.createQuery("select eno,ename,desg  from Employee where eno>=?1 and eno<=:no");
			...
			/*query=em.createQuery("select eno,ename,desg from Employee where ename in(:name1,:name2)");
			...
			/*query=em.createQuery("select eno from Employee where eno=:no");
			...
			/*query=em.createQuery("select eno,ename,desg from Employee where salary=(select max(salary) from Employee)");
			list=query.getResultList();
			for(Object obj1[]:list) {
				for(Object val:obj1) {
					System.out.print(val+"   ");
