	...
	public void loadAndUpdateData() {
		...
		try{
		    ...
		    if(tune!=null) {
		    	...
		    	tune.setCustName("don srinu");           // only one @ a time if multiple setter calls are written then only 1st one only participate in update 
		    	//tune.setCallerTune("rock on bro");
		    	//tune.setUpdationsCount(6);                  overides with hiberate last updationsCount with +1
		    	//tune.setCustId(126);								will not work with id field
		     	ses.update(tune);  
		    	flag=true;
		    }
		}
